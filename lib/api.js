...
class RPC {
  ...
  start(configPath, callback, unsafeFlag=false) {
    ...
    let logdir = null;
    if (!share.config.loggerOutputDirectory) {
      logdir = path.join(homedir(),'.config/storjshare/logs');
    } else {
      logdir = share.config.loggerOutputDirectory;
    }
    const fslogger = new FsLogger(logdir, nodeId);
