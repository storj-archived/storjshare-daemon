 ...
 * @param {String} config.farmerId - farmers unique identifier
 ...
 * @param {String} path - optional path to config
 *
 */
function ConfigManager(config, confPath) {
  if (!(this instanceof ConfigManager)) {
    return new ConfigManager(config, confPath);
  }

  if (!config || !config.farmerId) {
    throw new Error('A unique farmerId must be passed in');
  }

  this.id = config.farmerId;
  this.confPath = (confPath) ? confPath : path.join(
    CONFDIR, config.farmerId+'-settings.json'
  );
